...
test("rejects a missing database URL", async () => {
  await expect(runDatabaseProbe(undefined, "test-nextjs")).rejects.toThrow("DATABASE_URL is required");
});

test("rejects non-PostgreSQL URLs", async () => {
  await expect(runDatabaseProbe("https://example.com", "test-nextjs")).rejects.toThrow("must use PostgreSQL");
});

test.skipIf(!process.env.TEST_DATABASE_URL)("persists a counter across independent connections", async () => {
  const first = await runDatabaseProbe(process.env.TEST_DATABASE_URL, "test-nextjs");
  const second = await runDatabaseProbe(process.env.TEST_DATABASE_URL, "test-nextjs");
  expect(first).toMatchObject({ ok: true, framework: "test-nextjs", database: "postgresql" });
